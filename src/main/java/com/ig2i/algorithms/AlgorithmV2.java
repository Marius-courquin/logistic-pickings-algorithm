...
public class AlgorithmV2 implements Algorithm {
    ...
    private void addListOfArticleInZone(Instance instance, Zone zone, Article article){
        List<Article> articles = splitArticleInProduct(instance, article);
    ...
    private List<Article> splitArticleInProduct(Instance instance, Article article){
        List<Article> articles = new ArrayList<>();
        int remainingQuantity = article.getQuantity();

        while (remainingQuantity > 0) {
            Article newArticle = Article.of(1, article.getProduct(), article.getOrderId());
